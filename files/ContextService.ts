...
export class ContextService {
  ...
  private async initializeIndexes() {
    await this.ensureDirectories();
    const messageIndexPath = path.join(
      this.rootDir,
      "embeddings",
      "message_faiss.index"
    );
  ...
  private async initializeMemoryIndex() {
    ...
    for (const file of files) {
      if (file.isFile() && path.extname(file.name) === ".md") {
        ...
        if (data.uuid) {
          this.memoryIndexMap.set(data.uuid, {
            fileName: file.name,
            uuid: data.uuid,
          });
          this.memoryIndexMap.set(file.name, {
            fileName: file.name,
            uuid: data.uuid,
          });
        }
  ...
  private async rebuildMemoryIndex() {
    ...
    for (const [uuid, { fileName }] of this.memoryIndexMap.entries()) {
      ...
      const embedding = await this.openaiService.createEmbedding(
        memoryContent + " " + (data.keywords || []).join(" ")
      );
      ...
      await appendFile(
        metadataPath,
        JSON.stringify({ uuid, filename: fileName }) + "\n"
      );
    }
  ...
  async getRelevantContexts(
    similarMessages: Array<{ uuid: string }>,
    similarMemories: Array<{ uuid: string }>
  ) {
    ...
    return [...messageContexts, ...memoryContexts].filter(
  ...
  async saveConversation(config: {
    ...
    const cleanMessages = latestMessages.map(
    ...
    try {
      ...
      const customYamlParser = this.createCustomYamlParser();
      const { data: existingData, content: existingBody } = matter(
        existingContent,
        { engines: { yaml: customYamlParser } }
      );
      ...
      const updatedData = {
        ...
        keywords: Array.from(
          new Set([...(existingData.keywords || []), ...keywords])
        ).join(", "),
    ...
    const latestMessageEmbedding = await this.openaiService.createEmbedding(
      latestMessage.content
    );

    await this.addEmbeddingToIndex(
      latestMessageEmbedding,
      {
  ...
  async saveMemoryForConversation(
    memory: { title: string; content: string; keywords: string[] } | null,
    conversation_uuid: string
  ) {
    ...
    const hashtags = memory.keywords
    ...
    try {
      ...
      console.error(
        "Error updating conversation file with memory reference:",
        error
      );
      throw error;
    }

    const memoryEmbedding = await this.openaiService.createEmbedding(
      memory.content + " " + memory.keywords.join(" ")
    );
    await this.addEmbeddingToIndex(
      memoryEmbedding,
      {
  ...
  async searchSimilarMessages(
    embedding: number[],
    k: number = 5
  ): Promise<Array<{ uuid: string; role: string; similarity: number }>> {
  ...
  async searchSimilarMemories(
    embedding: number[],
    k: number = 5
  ): Promise<Array<{ uuid: string; similarity: number }>> {
    return this.searchSimilar(
      embedding,
      k,
      this.memoryIndex,
  ...
  private async searchSimilar(
    ...
    if (index.ntotal() === 0) {
      console.log(
        `The ${
          metadataFile.split("_")[0]
        } index is empty. No similar items found.`
      );
      return [];
    }

    const adjustedK = Math.min(k, index.ntotal());
    const { distances, labels } = index.search(
      this.normalizeVector(embedding),
      adjustedK
    );

    console.log(`FAISS search results for ${metadataFile}:`, {
      distances,
      labels,
    });
    ...
    const metadata = (
      await fs.readFile(
        path.join(this.rootDir, "embeddings", metadataFile),
        "utf-8"
      )
    )
      .split("\n")
    ...
    return labelsArray.flatMap((label, index) => {
      try {
        ...
        console.error(
          `Error parsing metadata for label ${label} in ${metadataFile}:`,
          error
        );
        return [];
      }
  ...
  async addEmbeddingToIndex(
    embedding: number[],
    metadata: { uuid: string; role?: string; filename: string },
    isMemory: boolean
  ) {
    const index = isMemory ? this.memoryIndex : this.messageIndex;
    const metadataFile = isMemory
      ? "memory_metadata.jsonl"
      : "message_metadata.jsonl";
    ...
    const indexPath = path.join(
      this.rootDir,
      "embeddings",
      isMemory ? "memory_faiss.index" : "message_faiss.index"
    );
  ...
  private async getAllConversations(): Promise<
    Array<{ conversation_uuid: string; messages: ChatCompletionMessageParam[] }>
  > {
    const conversations = [];
    const threadsDir = path.join(this.rootDir, "threads");
        ...
        for (const file of files) {
          if (file.isFile() && path.extname(file.name) === ".md") {
            ...
            try {
              ...
              console.error(
                `Error reading conversation file: ${filePath}`,
                error
              );
            }
          }
